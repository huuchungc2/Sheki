...
router.post('/requests/:id/approve', auth, authorize('admin'), async (req, res, next) => {
  try {
    ...
    // Load original order items with pricing to compute partial-return ratio
    const [orderItems] = await pool.query(
    ...
    for (const it of reqItems) {
      ...
      // Sum return base amount (for proportional commission adjustments)
      ...
      for (const line of list) {
        ...
        if (take > 0) {
          returnBaseTotal += computeReturnAmountForItem(line, take);
          remaining -= take;
        }
      }
    ...
    // Commission adjustments: reverse commissions proportionally for partial return
