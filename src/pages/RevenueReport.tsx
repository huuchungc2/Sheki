...
export function RevenueReport() {
  ...
  const performanceData = [...salesData].sort((a, b) => b.total_sales - a.total_sales);
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Thống kê doanh số và hiệu suất từng nhân viên.</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">{formatCurrency(summary.totalSales || 0)}</p>
            </div>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">{summary.totalEmployees || 0}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="h-[300px] w-full overflow-hidden">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={performanceData} layout="vertical" margin={{ left: 40 }}>
                  ...
                  <YAxis dataKey="full_name" type="category" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} />
                  <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }} formatter={(value: number) => [formatCurrency(value), 'Doanh số']} />
                  <Bar dataKey="total_sales" radius={[0, 4, 4, 0]} barSize={20}>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-900">Chi tiết doanh số nhân viên</h2>
            </div>
      ...
      {salesData.length === 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
          <p className="text-slate-400">Chưa có dữ liệu doanh thu trong tháng này</p>
        </div>
      )}
    </div>
  );
