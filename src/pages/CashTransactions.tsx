    ...
    <div ref={wrapRef} className={cn("relative", className)}>
      <div className="relative w-full">
        ...
        <input
          ...
          autoComplete="off"
          className="w-full rounded-md border border-input bg-background pl-9 pr-9 py-2.5 sm:py-2 text-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background min-h-[44px] sm:min-h-0"
        />
        {q ? (
          <button
            type="button"
            onClick={clear}
            className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent transition-colors text-lg leading-none"
            aria-label="Xóa"
          >
            ×
          </button>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full max-h-56 overflow-y-auto rounded-lg border border-border bg-popover text-popover-foreground shadow-lg">
          {filtered.length === 0 ? (
            <div className="px-3 py-2.5 text-sm text-muted-foreground">Không tìm thấy nhân viên</div>
          ) : (
            filtered.map((e) => (
              <button
                key={e.id}
                ...
                onClick={() => pick(e)}
                className="w-full text-left px-3 py-2 text-sm hover:bg-accent/50 border-b border-border last:border-0 transition-colors"
              >
...
export function CashTransactions() {
  ...
  return (
    <div className="space-y-6 px-1 sm:px-0">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Thu chi</h1>
      ...
      <div className="bg-card rounded-xl border border-border p-4 md:p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="md:col-span-2 lg:col-span-1">
            <label className="block text-xs font-medium text-muted-foreground mb-1">Nhân viên *</label>
            {isAdmin ? (
              <EmployeeSearchCombo
                employees={employees}
                valueId={userId}
              ...
              <div className="rounded-md border border-border bg-muted/30 px-3 py-2.5 sm:py-2 text-sm min-h-[44px] flex items-center">
              ...
              <div className="mt-2">
                {userGroups.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5 items-center">
                    <span className="text-[11px] font-medium text-muted-foreground shrink-0">Nhóm BH:</span>
                    {userGroups.map((g) => (
                      <span
                        key={g.id}
                        className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold bg-muted/30 text-foreground border border-border"
                      >
                        {g.name}
                      </span>
                    ))}
                  ...
                  <p className="text-[11px] text-amber-700 dark:text-amber-300">Chưa gán nhóm bán hàng cho nhân viên này.</p>
                )}
              </div>
            ) : null}
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Loại *</label>
            <select
              value={kind}
              onChange={(e) => setKind(e.target.value as "income" | "expense")}
              className="w-full rounded-md border border-input bg-background px-3 py-2.5 sm:py-2 text-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background min-h-[44px] sm:min-h-0"
            >
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Nhóm bán hàng</label>
            <select
              value={groupId}
              onChange={(e) => setGroupId(e.target.value)}
              disabled={!userId}
              className="w-full rounded-md border border-input bg-background px-3 py-2.5 sm:py-2 text-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:bg-muted/30 disabled:text-muted-foreground min-h-[44px] sm:min-h-0"
            >
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Số tiền (VNĐ) *</label>
            <input
              type="text"
              inputMode="numeric"
              value={amount}
              ...
              className="w-full rounded-md border border-input bg-background px-3 py-2.5 sm:py-2 text-base sm:text-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background min-h-[44px] sm:min-h-0 tabular-nums"
            />
          </div>
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background resize-y min-h-[72px]"
            />
          </div>
        </div>
        <button
          ...
          disabled={submitting}
          className={cn(
            "w-full sm:w-auto justify-center inline-flex items-center gap-2 rounded-md px-4 py-2.5 sm:py-2 text-sm font-semibold text-primary-foreground min-h-[44px] sm:min-h-0",
            "bg-primary hover:opacity-95 transition-opacity disabled:opacity-50 active:opacity-90"
          )}
        >
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <div className="p-4 border-b border-border flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
            <h2 className="text-sm font-semibold text-foreground">Danh sách</h2>
            <button
              type="button"
              onClick={handleExportExcel}
              disabled={exporting}
              className={cn(
                "inline-flex items-center justify-center gap-2 rounded-md border border-border bg-background px-3 py-2 text-sm font-semibold text-foreground min-h-[44px] sm:min-h-0",
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-2 items-stretch sm:items-center">
            ...
            <select
              ...
              className="w-full sm:w-auto rounded-md border border-input bg-background px-3 py-2 sm:py-1.5 text-sm min-h-[44px] sm:min-h-0"
            >
            ...
            <div className="flex gap-2 items-center w-full sm:w-auto">
              <select
                ...
                className="flex-1 sm:flex-initial rounded-md border border-input bg-background px-2 py-2 sm:py-1.5 text-sm min-h-[44px] sm:min-h-0"
                aria-label="Tháng"
              >
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="font-semibold text-foreground text-sm mt-0.5">{r.user_full_name}</p>
                    <p className="text-xs text-muted-foreground">{r.user_username}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      type="button"
                      onClick={() => remove(r.id)}
                      className="p-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 min-w-[44px] min-h-[44px] flex items-center justify-center transition-colors"
                      title="Xóa"
                      aria-label="Xóa"
                    >
                ...
                <div className="text-[11px] text-muted-foreground pt-1 border-t border-border space-y-1">
                  <p>
                    <span className="text-muted-foreground">Nhóm:</span> {r.group_name || "—"}
                  </p>
                  <p>
                    <span className="text-muted-foreground">Người tạo:</span> {r.created_by_name}
                  </p>
                  {r.note ? (
                    <p className="break-words">
                      <span className="text-muted-foreground">Ghi chú:</span> {r.note}
                    </p>
                  ) : null}
                </div>
                <p
          ...
          <table className="w-full text-sm min-w-[720px]">
            <thead>
              <tr className="bg-muted/30 text-left text-muted-foreground">
                ...
                <th className="px-4 py-3 font-semibold whitespace-nowrap">Nhân viên</th>
                <th className="px-4 py-3 font-semibold whitespace-nowrap">Loại</th>
                ...
                <th className="px-4 py-3 font-semibold whitespace-nowrap">Người tạo</th>
                ...
                rows.map((r) => (
                  <tr key={r.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-2.5">
                      <div className="font-semibold text-foreground">{r.user_full_name}</div>
                      <div className="text-xs text-muted-foreground">{r.user_username}</div>
                    </td>
                    <td className="px-4 py-2.5">
                      <span
                        className={cn(
                    ...
                    <td className="px-4 py-2.5 text-muted-foreground max-w-[200px] truncate" title={r.note || ""}>
                      {r.note || "—"}
                    </td>
                    ...
                    <td className="px-4 py-2.5">
                      <button
                        type="button"
                        onClick={() => remove(r.id)}
                        className="p-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 min-w-[40px] min-h-[40px] inline-flex items-center justify-center transition-colors"
                        title="Xóa"
                        aria-label="Xóa"
                      >
                        <Trash2 className="w-4 h-4" />
        ...
        {total > limit && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 border-t border-border bg-muted/30">
            ...
            <div className="flex gap-2 justify-center">
              <button
                ...
                className="p-3 sm:p-2 rounded-md border border-border disabled:opacity-40 min-w-[44px] min-h-[44px] sm:min-w-0 sm:min-h-0 flex items-center justify-center bg-background hover:bg-accent transition-colors"
