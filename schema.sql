...
CREATE TABLE `orders` (
  ...
  `is_counter_sale` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1 = Đơn bán tại quầy / tại cửa hàng',
  ...
  INDEX `idx_orders_counter_sale` (`is_counter_sale`),
...
CREATE TABLE IF NOT EXISTS `shops` (
  ...
  `order_line_show_commission` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=hiện cột HH OrderForm / đơn giao',
  `order_default_commission_rate` DECIMAL(5,2) NOT NULL DEFAULT 10.00 COMMENT '% HH mặc định dòng mới (đơn giao)',
  `order_line_show_discount` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=hiện cột CK% OrderForm / đơn giao',
  `order_default_discount_rate` DECIMAL(5,2) NOT NULL DEFAULT 0.00 COMMENT '% CK mặc định dòng mới (đơn giao)',
  `counter_order_line_show_commission` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=hiện cột HH bán tại quầy',
  `counter_order_line_show_discount` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=hiện cột CK% bán tại quầy',
  `counter_order_default_discount_rate` DECIMAL(5,2) NOT NULL DEFAULT 0.00 COMMENT '% CK mặc định dòng mới (quầy)',
  `counter_order_default_commission_rate` DECIMAL(5,2) NOT NULL DEFAULT 10.00 COMMENT '% HH mặc định dòng mới (quầy)',
  `order_qty_allow_decimal` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=SL nhập lẻ (DECIMAL 10,3), 0=chỉ số nguyên (đơn giao)',
  `counter_order_qty_allow_decimal` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=SL nhập lẻ (DECIMAL 10,3), 0=chỉ số nguyên (quầy)',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
