...
- **Cách hạch toán hoa hồng khi hoàn:** khi Admin duyệt hoàn, hệ thống tạo **bút toán điều chỉnh hoa hồng** (`commission_adjustments`) giá trị **âm** để đảo lại đúng phần hoa hồng đã ghi nhận trên hàng bị hoàn. Hoàn một phần tính theo từng dòng sản phẩm/số lượng từ snapshot `order_items.commission_amount` + `order_items.commission_rate`; hoàn toàn bộ dùng cùng công thức với đủ số lượng và quyết toán đúng phần HH gốc còn lại. HH quản lý dùng `commissions.override_rate` đã lưu khi đơn có một mức, hoặc tra tier theo `order_items.commission_rate` đã lưu cho đơn nhiều mức. Tổng các lần hoàn không được vượt HH direct/override gốc.
